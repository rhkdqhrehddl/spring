...
<%
	...
	String LGD_INSTALLRANGE			= request.getParameter("LGD_INSTALLRANGE");				//할부개월
	String LGD_USABLECARD			= request.getParameter("LGD_USABLECARD");				//결제 신용카드
	String LGD_EASYPAY_ONLY			= request.getParameter("LGD_EASYPAY_ONLY");				//paynow 사용여부
	String LGD_MONEPAY_AUTORUNYN	= request.getParameter("LGD_MONEPAY_AUTORUNYN");		//paynow 자동구동 여부
	String LGD_PCVIEWYN				= request.getParameter("LGD_PCVIEWYN");					//휴대폰번호 입력 화면 사용 여부(유심칩이 없는 단말기에서 입력-->유심칩이 있는 휴대폰에서 실제 결제)

	
	
	/*
     * 가상계좌(무통장) 결제 연동을 하시는 경우 아래 LGD_CASNOTEURL 을 설정하여 주시기 바랍니다. 
     * 인증결과 수신페이지 (상점정의에 맞게 변경해주세요)
     */  
     String LGD_CASNOTEURL		= "https://rhkdqhrehddl.tk/PG/SmartXPayEasyPay/cas_noteurl.do";  
	
	/*
	 * LGD_RETURNURL 을 설정하여 주시기 바랍니다. 반드시 현재 페이지와 동일한 프로트콜 및  호스트이어야 합니다. 아래 부분을 반드시 수정하십시요.
	 * 인증결과 수신페이지 (상점정의에 맞게 변경해주세요)
	 */    
	 String LGD_RETURNURL		= "https://rhkdqhrehddl.tk/PG/SmartXPayEasyPay/returnurl.do";     
	 ...
	 */ 
	String LGD_KVPMISPWAPURL		= "";		//결제성공결과 화면 페이지(상점정의에 맞게 변경해주세요)
	String LGD_KVPMISPCANCELURL     = "";		//결제중단 결과페이지 (상점정의에 맞게 변경해주세요)
	
	String LGD_MPILOTTEAPPCARDWAPURL = "";		//iOS에서 연동시 필수
	...
	payReqMap.put("CST_MID"						, CST_MID );							// LG유플러스로 부터 발급받으신 상점아이디를 입력하세요.
	payReqMap.put("LGD_MID"						, LGD_MID );                        	// 테스트 아이디는 't'를 제외하고 입력하세요.
	...
	payReqMap.put("LGD_VERSION"					, "JSP_Non-ActiveX_SmartXPay_Paynow");	// 사용타입 정보(수정 및 삭제 금지): 이 정보를 근거로 어떤 서비스를 사용하는지 판단할 수 있습니다.
	...
	*/
	payReqMap.put("LGD_KVPMISPAUTOAPPYN"	, "A");					// 신용카드 결제 
	payReqMap.put("LGD_MTRANSFERAUTOAPPYN"	, "A");					// 계좌이체 결제
